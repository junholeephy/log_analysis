...
| `run.py` | 구 파이프라인. case20/case21 판별만, 라벨 6개 | **회귀 기준선** |
...
정책상 거절이 아니다. 이걸 거절로 읽으면 case27 으로 빠지고 **진짜 원인이 통계에서
사라진다** — 실제로 났던 사고다.
...
answer_used_rag=ignored            -> 답변이 쓰지 않았다
                                   v
case21  Retrieve 성공, 생성 실패    (TYPE5 / category_2, 신뢰도 medium)
```

**case20(Retrieve 실패)가 아니라 case21 인 것이 이 도구의 존재 이유다.** 사용자에게는
똑같이 "답이 부실하다"로 보이지만, case20 면 문서를 써야 하고 case21 이면 프롬프트를
고쳐야 한다. 멀쩡한 코퍼스에 문서를 더 채우는 헛수고를 막는 것이 이 구분이다.

`category_2` 는 case21 에서 계산된다. 따로 분류하지 않는다.
...
```json
{
  ...
  "classification": {
    "case_id": "case21", "case_name": "Retrieve 성공, 생성 실패",
...
| **`answer_refused` 가 회피를 거절로 읽음** | **case21 탐지가 무너짐** | 정책·권한·보안으로 좁힘 |
| **`answer_refused` 가 서비스 장애 문구를 거절로 읽음** | **인프라 장애가 case27(보안 정책)로 집계** | 확정 문구를 코드로 대조해 LLM 이전에 단락 (case9) |
