...
# "짧게 답해줘" 의 기준값 (case11).
#
# **지금은 판정에 쓰지 않는다.** 길이는 기준이 사용자 머릿속에 있어 코드로
# 가릴 수 없다 - check_length 는 재기만 한다. 503건 실측에서 이 값(400)은 최대
# 답변이 370자인 데이터라 한 번도 걸리지 않았고, 걸렸더라도 근거 없는 임의값이었다.
#
# 키를 남겨 두는 이유: 실데이터에서 답변 길이 분포를 본 뒤에 "이 조직에서 짧게는
# 몇 자인가"를 정할 수 있고, 그때 판정을 켜는 자리가 여기다.
