...
- none            아무것도 문제 삼지 않는다 (아래 설명을 읽어라)
...
**none 을 쓸 때.** 후속 발화가 앞 답변을 문제 삼지 않으면 none 이다. 이건 흔하다 —
이 로그는 넓게 골라온 것이라 정상 대화가 섞여 있다.
- none: "그럼 반차는 어떻게 되나요?" (앞 답변을 받아들이고 다음을 묻는다)
        "네 감사합니다", "그렇군요. 그러면 신청은 어디서 하나요?"
- none 아님: "그건 아까 물어본 거잖아요", "그래서 며칠이라는 건가요?"
             (앞 답변이 부족했다는 신호가 발화에 있다)

가르는 기준은 **주제가 바뀌었는가가 아니라 앞 답변을 문제 삼는가**다. 같은 주제를
더 파고드는 것도, 그 자체로 앞 답변을 탓하지 않으면 none 이다.

complaint_quote — 그렇게 읽은 근거를 후속 발화에서 **글자 그대로** 따와라.
none 일 때 특히 그렇다. 지어낸 인용은 원문 대조에서 걸러지고, 걸러지면 none 은
무효가 되어 불만으로 되돌아간다. 근거를 댈 수 없으면 none 이 아니다.

question_domain — 마지막 질문이 어떤 종류인가:
- domain            업무 문서를 찾아야 답할 수 있다 (규정·절차·제도)
...
운영 환경 챗봇에서는 후자가 훨씬 흔하다. 애매하면 domain 이다 — 공개 지식으로 답할 수
