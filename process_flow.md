...
| 주는 것 | 안 주는 것 |
|---|---|
| 이전 질문들 (최근 3개) | **`rag_chunks`** ← 이게 핵심 |
| 마지막 질문에 대한 챗봇 답변 | 부서 · 직급 · 직무 |
| 그 답변에 대한 사용자의 후속 발화 | `llm_eval_*` · `llm_emotion_*` |
| | turn 번호 · timestamp |
...
| 주는 것 | 안 주는 것 |
|---|---|
| ⑤가 정리한 질문 (`resolved_question`) | **`llm_ans_on_last_q`** ← 이게 핵심 |
| ⑤가 정리한 원했던 정보 (`unmet_need`) | `current_query` (불만 원문) |
| 그때 검색된 문서 (번호 붙여서) | 이전 질문들 |
...
그래서 case14(이전 턴 맥락 상실)를 앞에 두면 case20 · case22 를 가로챈다 — 구현 중 실제로
회귀셋 6건이 그렇게 샜다. **인용으로 검증된 문서 증거가 LLM 의 인상보다 강하다.**
case14 는 9번에서만 주 라벨이 되고, 그 전에는 부가 케이스로만 붙는다.
...
|---|---|---|---|---|
| ⑤ 관측 | 무엇을 원했나 | 질문 · 답변 · 불만 | **문서** | 문서에 맞춰 요구를 재구성하는 순환 |
| ⑦ 충족도 | 문서에 있었나 | 질문 · 원한 것 · 문서 | **답변** | "답변이 나쁨"을 "문서가 나쁨"으로 읽음 |
| ⑨ 근거활용 | 답변이 썼나 | 답변 · 문서 | **질문 · 불만** | ⑦과 같은 방향으로 쏠린 두 번째 표 |
