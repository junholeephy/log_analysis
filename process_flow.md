...
| 6 | 언어·길이·포맷 요구를 지켰나 | 코드 | `case10` `case11` `case12`<br>지켰는데도 불만이면 `case13` |
...
라우팅의 대부분은 단일 조건이라 위 표로 끝나지만, TYPE5 분기만 값 네 개가
조합된다. 이 부분이 이 도구의 핵심이라 전개해 둔다. 아래는 `route.py` 를
실제로 돌려 만든 것이다.
...
`n_chunks == 0` 인데 `sufficient` 가 나오는 조합은 실제로는 생기지 않는다.
청크가 비면 ⑦을 건너뛰고 `insufficient` 로 두기 때문이다(⑦의 "청크가 비어 있으면"
참고). 표에서 뺀 이유다.
...
```
"출장비는 규정에 따라 지급됩니다"        → case17  (틀리진 않았다. 그런데 뭘 하라는 건지)
