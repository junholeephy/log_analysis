...
1. 데이터와 판정 프롬프트를 같은 사람이 만들면 편향을 공유한다. 여기서 나온
   일치율은 실전보다 후하다.
2. 합성 문서는 지어낸 사내 규정이라 판정자가 사전지식으로 알 리가 없다.
...
CASES = [
    ...
    dict(
        ...
        complaint="그게 우리 회사 사규에도 그렇게 되어 있나요? 사내 규정 근거를 보여주세요.",
        chunks=[
    ...
    dict(
        ...
        answer="복지포인트는 사내 복지몰, 제휴 가맹점, 도서 구입, 문화 활동 등에 사용할 수 있습니다.",
        complaint="답변이 너무 짧아요. 좀 더 친절하게 풀어서 설명해주면 안 되나요?",
        chunks=[
            "복지포인트는 사내 복지몰과 제휴 가맹점에서 사용할 수 있다.",
    ...
    dict(
        ...
        pre_queries=["사내 VPN 접속이 안 되는데요.", "MFA 기기를 바꿨는데 VPN 인증이 계속 실패해요."],
        answer="VPN 접속 문제는 대부분 네트워크 설정 때문입니다. 클라이언트를 재설치해 보세요.",
        complaint="MFA 기기 변경했을 때 재등록 절차를 알려달라니까요.",
        chunks=[
            "VPN 클라이언트 설치 및 초기 설정 안내: 사내 포털에서 설치 파일을 내려받아 실행한다.",
            "VPN 접속 오류 시 네트워크 어댑터를 재설정하고 클라이언트를 재시작한다.",
            "MFA는 전사 필수 정책이며 모든 사내 시스템 접근에 적용된다.",
        ],
    ),
    dict(
        ...
        context_dependent=True,
        pre_queries=["사내 GPU 서버 사용 신청 어떻게 하나요?", "신청하면 얼마나 쓸 수 있어요?"],
        ...
        chunks=[
            ...
            "신청은 사내 인프라 포털에서 진행한다.",
        ],
    ),
    dict(
        user="u3", conv="C-3002", turn=5, trap="distractor", expect=["rag_insufficient"],
        pre_queries=["사내 계정 비밀번호 정책이 어떻게 되나요?", "비밀번호 변경 주기가 몇 개월인가요?"],
    ...
    dict(
        ...
        pre_queries=["HTTPS 기본 포트가 몇 번인가요?", "사내 서버 방화벽에서 열려 있는 포트요."],
        answer="HTTPS는 443 포트를 사용합니다.",
        complaint="일반론 말고, 사내 방화벽에서 실제로 허용된 포트 목록을 알려주세요.",
        chunks=[
    ...
    dict(
        ...
        context_dependent=True,
        pre_queries=["사내 메일 용량 초과 시 해결 방법 알려주세요.", "단계별로 정리해줄 수 있나요?"],
        answer="사내 메일 시스템의 용량이 초과되는 경우에는 여러 가지 방법을 고려해 볼 수 있는데, "
    ...
    dict(
        ...
        complaint="사내 문서 기준으로 확인해주세요. 영세율 대상도 있을 텐데요.",
        chunks=[
...
#
# 이 셋은 case17/18 판별 전용으로 만들어졌고 실제 LLM 으로 23/23 검증된 유일한
# 케이스 집합이다. 새 파이프라인이 같은 결과를 내는지 확인하는 데 쓴다.
#
# 1:1 이 아니다. out_of_scope 하나가 형식·길이·언어로 쪼개졌고, rag_partial 은
# case17 에 흡수됐다. 쪼개진 쪽은 케이스마다 expect_case 로 직접 지정한다.
# ---------------------------------------------------------------------------

LEGACY_TO_CASE = {
    "rag_insufficient": {"case17"},
    "rag_partial": {"case17"},                       # 신 체계에서는 둘 다 case17
    "rag_sufficient_generation_failed": {"case18"},
...
EXPECT_CASE = {
    ...
    # rag_partial 은 신 체계에서 case17 이므로 그쪽도 정답이다.
    "C-2004:4": {"case13", "case17"},
    "C-2003:11": {"case11"},                 # "답변이 너무 짧아요" -> 길이
    "C-3003:9": {"case12"},                  # "번호 매겨서" -> 포맷
    # "말투가 딱딱하고 영어 용어가 많다" -> 톤·용어. complaint_target 에 딱 맞는 값이
    # 없어 format 으로 갈 수도, taxonomy 밖으로 갈 수도 있다. 둘 다 인정한다.
    "C-4003:13": {"case12", "out_of_taxonomy", "unclassified"},
}
