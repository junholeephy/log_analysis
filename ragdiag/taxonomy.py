...
class TaxonomyCase:
    ...
    # (case3/case14, case4/case13, case12/case16, case19/case20) 가르는 기준을 함께 적는다.
    desc: str = ""
...
# 라벨이 case19 같은 번호로만 남고, 번호만 보고 무엇인지 아는 사람은 없다.
#
...
_DESC = {
    ...
    "case2":  "그림·사외 링크처럼 이 서비스가 낼 수 없는 형태를 요구했다. "
              "낼 수 있는 형식인데 안 지킨 것은 case11.",
    "case3":  "한 번에 여러 의도를 섞어 물었다 — 사용자 쪽 문제다. "
              "모델이 그중 일부만 답한 것은 case14.",
    "case4":  "'그거 다시 알려줘'처럼 앞 턴을 전제로 지시 대상이 불명확하다. "
              "질문이 불완전한 것이고, 답변이 맥락을 잃은 것은 case13.",
    ...
    "case9":  "특정 언어로 답해 달라고 했는데 다른 언어로 답했다.",
    "case10": "짧게/길게 등 분량 요구를 지키지 않았다.",
    "case11": "표·불릿 등 요구한 형식으로 내지 않았다. "
              "애초에 낼 수 없는 형식을 요구한 것은 case2.",
    "case12": "물은 것과 다른 것을 답했다. 의도 파악 실패. "
              "맞게 답했으나 실행으로 이어지지 않는 것은 case16.",
    "case13": "답변이 앞 턴의 맥락을 잊거나 잘못 이었다. "
              "검색이 실패해도 이렇게 보이므로, 문서 증거가 있는 case19·case20 을 먼저 가른다.",
    "case14": "여러 의도 중 일부만 답했다 — 고칠 곳은 모델이다. "
              "사용자가 복합 질문을 한 것 자체는 case3.",
    "case15": "말투·어조·용어 사용에 대한 요구를 지키지 않았다.",
    "case16": "내용은 맞으나 사용자가 다음에 무엇을 할지 알 수 없다. "
              "아예 다른 것을 답한 것은 case12.",
    "case17": "검색된 문서와 어긋나는 주장을 했다. "
              "문서 밖의 허구는 대조할 것이 없어 판정 대상이 아니다.",
    "case18": "같은 질문에 매번 다르게 답하거나 이전 답변과 상충한다. "
              "턴 하나가 아니라 로그 전체를 훑어야 해 이 파이프라인에서는 판정하지 않는다.",
    "case19": "가져온 청크에 답이 없다. 검색기가 못 찾은 것인지 문서가 애초에 없는 것인지는 "
              "코퍼스 전체를 봐야 갈린다 — 특정 부서에 몰리면 문서 부재 쪽이다.",
    "case20": "청크에 답이 있는데 답변이 쓰지 않았다. "
              "'인사팀에 문의하세요' 같은 회피성 안내도 거절이 아니라 여기다.",
    "case21": "최신 문서 대신 구 문서를 가져왔다. "
              "청크에 문서 ID·개정일이 없어 판정하지 않는다.",
    "case22": "답변이 인용부호로 제시한 문장이 문서와 대조되지 않는다. "
              "문서명·조항번호는 청크에 메타데이터가 없어 검증 범위 밖이다.",
    "case23": "상식 질문에 틀리게 답했다. 판정 근거가 판정자의 사전지식뿐이라 "
              "다른 케이스와 같은 무게로 집계하면 안 된다.",
    "case24": "답변 안의 등식을 다시 계산해 보니 틀렸다. "
              "'5영업일 뒤면 3월 13일' 같은 자연어 계산은 판정 범위 밖이다.",
    "case25": "SQL·Python·Excel 등의 사용법이나 코드가 부정확하다. "
              "문법까지만 검증되고 실행 결과는 보지 않는다.",
    "case26": "보안 정책상 답할 수 없는 정보를 요청해 거절했다. "
              "권한 부족으로 거절한 경우와 로그상 구분되지 않는다.",
    "case27": "문서 안에 숨은 지시를 모델이 그대로 수행했다. "
...
def label(case_id: str) -> str:
    """`case11 · 요구 포맷 불이행` — 번호만으로는 무엇인지 알 수 없다."""
