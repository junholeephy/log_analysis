...
def secondary_from(obs: Observation, checks: dict[str, Check]) -> list[str]:
    ...
    한 턴이 case4(모호한 질문)이면서 동시에 case19(검색 실패)일 수 있다. 억지로
...
def route(
    ...
    if obs.answer_refused:
        ...
        return done("case26", "답변이 정책·권한을 이유로 거절함",
    ...
    if injection is not None and injection.violated:
        return done("case27", f"검색 문서의 지시를 답변이 수행함 — {injection.detail}")
    ...
    for target, check_name, case_id in REQUEST_CASES:
        ...
        if broken is False:
            ...
            return done("case12", f"{target} 요구는 지켰으나 사용자가 불만",
    ...
    if obs.question_domain in ("code", "tool_usage"):
        for name in ("python_syntax", "sql_shape"):
            ...
            if broken is not None and broken.violated:
                return done("case25", f"코드 결함 확인 — {broken.detail}")
        return done("case25", "코드·도구 질문에 대한 불만",
    ...
    # --- 6. 남은 것 -----------------------------------------------------------
    # case13 는 여기서만 주 라벨이 된다. 앞에 두면 case19/case20 을 가로챈다 —
    # 검색이 실패해 답변이 부실하면 모델은 그걸 "히스토리를 못 이어받았다"로도
    # 읽기 때문이다. 인용으로 검증된 문서 증거가 LLM 의 인상보다 강하다.
    if obs.answer_used_history == "ignored":
        return done("case13", "답변이 이전 턴의 내용을 잊거나 잘못 연결함",
                    ["더 구체적인 원인을 찾지 못해 맥락 상실로 판정"])

    if obs.question_multi_intent and not obs.answer_covers_all_intents:
        return done("case14", "복합 질문의 일부만 답변함")
...
def _route_domain(obs, judgment, citation, grounding, done) -> Classification:
    """TYPE5 분기 — 검증된 case19/case20 판별 로직을 그대로 쓴다."""
    if judgment is None:
    ...
    if verdict in ("insufficient", "partial"):
        ...
        return done("case19", f"문서가 요구를 충족하지 못함 (verdict={verdict})", note)

    if grounding is None:
        return done("case19", "문서는 충분하나 활용 여부를 확인하지 못함")

    if grounding.answer_used_rag == "ignored":
        return done("case20", "문서에 답이 있는데 답변이 쓰지 않음")
    if grounding.answer_used_rag == "contradicted":
        return done("case17", "답변이 문서와 어긋나는 주장을 함",
    ...
    if not obs.answer_actionable:
        ...
        return done("case16", "문서는 충분하나 답변이 실행할 수 있는 수준이 아님")
    return done("case12", "문서는 충분하고 활용했으나 사용자 기대와 다름")
